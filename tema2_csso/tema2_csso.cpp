...
int List_Processes() {
    ...
    {
        cout << "PPID = " << pe32.th32ParentProcessID << " | PID = " << pe32.th32ProcessID << " | Name = " << pe32.szExeFile<<"\n";
    }
    CloseHandle(hProcessSnap);
    return TRUE;
}
...
int process1(const char* pathExe, string pathRead, PROCESS_INFORMATION &processInformation)
{
    STARTUPINFO startupInfo;
    ZeroMemory(&startupInfo, sizeof(startupInfo));
    startupInfo.cb = sizeof(startupInfo);

    if (!CreateProcess(pathExe, LPSTR(pathRead.c_str()), NULL, NULL, FALSE, 0, NULL, NULL, &startupInfo, &processInformation))
    {
...
int process2(string path,string pathWrite, PROCESS_INFORMATION &processInformation)
{
    STARTUPINFO startupInfo;
    ZeroMemory(&startupInfo, sizeof(startupInfo));
    startupInfo.cb = sizeof(startupInfo);
    if (!CreateProcess(path.c_str(), (LPSTR)(pathWrite.c_str()), NULL, NULL, FALSE, NULL, 0, 0, &startupInfo, &processInformation))
    {
        cout << "Create process error";
        CloseHandle(processInformation.hThread);
        CloseHandle(processInformation.hProcess);
...
int main()
{
    ...
    else if (choose == 6)
    {
        ...
        process1("C:\\Users\\bujor\\source\\repos\\process1\\x64\\Debug\\process1.exe", pathRead, processInformation1);
        process2("C:\\Users\\bujor\\source\\repos\\process2\\x64\\Debug\\process2.exe", pathWrite, processInformation2);
