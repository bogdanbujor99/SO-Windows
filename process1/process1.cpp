...
int main(int argc, char* argv[])
{
    if (argc < 1)
    {
        cout << "Numar prea mic de parametri";
    }
    HANDLE hfile;
    char s[256];
    if (!(hfile = CreateFile(argv[0], GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)))
    {
    ...
    if (!ReadFile(hfile, s, fileSize.QuadPart, &byte, NULL))
    {
        printf("Error: %d", GetLastError());
    };
